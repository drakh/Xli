...
namespace Xli
{
    ...
    */
    ...
    public:
        ...
        template <typename T> void Write(const Vector2t<T>& vec) { _stream->WriteSafe(vec.DataPtr(), sizeof(T), 2); }
        template <typename T> void Write(const Vector3t<T>& vec) { _stream->WriteSafe(vec.DataPtr(), sizeof(T), 3); }
        template <typename T> void Write(const Vector4t<T>& vec) { _stream->WriteSafe(vec.DataPtr(), sizeof(T), 4); }
